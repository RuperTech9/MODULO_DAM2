...
public class E07_LecturaCancionesXml {
    public static void main(String[] args) {
        DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();

        try {
            DocumentBuilder builder = factory.newDocumentBuilder();
            Document document = builder.parse(new File("./src//AccesoADatos//T01_Ficheros/TareaFicheroAleatorioCanciones/canciones.xml"));
            ...
            System.out.printf("Nodos canción a recorrer: %d %n",
                    canciones.getLength());
            ...
            for (int i = 0; i < canciones.getLength(); i++) {
                ...
                if (cancion.getNodeType() == Node.ELEMENT_NODE) { // tipo de nodo es "ELEMENT_NODE"
                    ...
                    System.out.printf("Año = %s%n",
                            elemento.getElementsByTagName("anio").item(0).getTextContent());
                    System.out.printf("Título = %s%n",
                            elemento.getElementsByTagName("titulo").item(0).getTextContent());
                    System.out.printf("Artista = %s%n",
                            elemento.getElementsByTagName("artista").item(0).getTextContent());
                    System.out.printf("Duración = %s%n",
                            elemento.getElementsByTagName("duracion").item(0).getTextContent());
                    System.out.printf("Es Española = %s%n",
                            elemento.getElementsByTagName("esEspanola").item(0).getTextContent());
                    System.out.println("----------------------------");
                }
            }
        } catch (Exception e) {
